...
class Abovethefold_LocalizeJSModule_GoogleWebfont extends Abovethefold_LocalizeJSModule {
	...
	public function parse_webfont_js( $code ) {
		...
		if (strpos($code,'webfont.js') !== false && preg_match_all('|googleapis\.com/[a-z\/]+/([^/]+)/webfont\.js|Ui',$code,$out)) {
			...
			if ($this->CTRL->options['localizejs'][$this->classname]['incmethod'] === 'replace') {
				...
				// ga.src regex
				$regex[] = '#\.src\s*=\s*\([^\)]*\)\s*\+\s*[\'|"][^\'"]*ajax\.googleapis\.com/[a-z\/]+/([^/]+)/webfont\.js[\'|"];#Ui';
				$replace[] = '.src = \'' . $script_url . '\';';

				// full url regex
				$regex[] = '#([\'|"])[^\'"]*ajax\.googleapis\.com/[a-z\/]+/([^/]+)/webfont\.js[\'|"]#Ui';
				 ...
				 */
				$regex[] = '#\(\s*function\(\s*\)((?!</script).)*googleapis\.com/[a-z\/]+/([^/]+)/webfont\.js((?!</script).)*\(\s*\)\;#is';
