...
/**
 * Facebook conversion pixel (fbds.js)
 *
 * @since      2.3.3
...
class Abovethefold_LocalizeJSModule_FacebookFbds extends Abovethefold_LocalizeJSModule {


	// The name of the module
	public $name = 'Facebook conversion pixel (fbds.js)';
	public $link = 'https://developers.facebook.com/docs/ads-for-websites/drive-conversions#install-pixel';

	public $update_interval = 86400; // once per day
	public $script_source = 'http://connect.facebook.net/%%LANG%%/fbds.js';
	...
	public function enqueue_script( ) {
		...
		wp_enqueue_script( 'facebook-fbds-js', $script_url , array(), date('Ymd', $script_time) );
	...
	public function parse_facebook_sdk_js( $code ) {
		...
		if (preg_match_all('|connect\.facebook\.net/([^/]+)/fbds\.js|Ui',$code,$out)) {
			...
			if ($this->CTRL->options['localizejs'][$this->classname]['incmethod'] === 'replace') {
				...
				$regex[] = '#([\'|"])((http(s)?)?(:)?//connect\.facebook\.net/([^/]+)/fbds\.js(\#[^\'"]+)?)[\'|"]#Ui';
				 ...
				 */
				$regex[] = '#\(function\(\)\{[^\{]+\{[^\}]+connect\.facebook\.net/([^/]+)/fbds\.js[^\}]+\}[^\}]+\}\)\(\);#is';
